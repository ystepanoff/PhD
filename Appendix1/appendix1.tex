...
sum
\begin{equation}
V = \langle e_1, f_1 \rangle \oplus \ldots \oplus
\langle e_{m-1}, f_{m-1} \rangle \oplus 
W_2,	
\end{equation}
where $(e_1,f_1),\ldots,(e_{m-1},f_{m-1})$ are
...
the basis $\mathcal{B} = \{v_1, w_1,\ldots,w_{2m}, v_2\}$ in $V$ such that $(v_1,v_2)$ 
is a hyperbolic pair. Consider the decomposition $V = \langle v_1 \rangle \oplus
\langle w_1, \ldots, w_{2m} \rangle \oplus \langle v_2 \rangle$ and denote 
$W = \langle w_1, \ldots, w_{2m} \rangle$. Further, denote by $Q_W$ and $f_W$ the restrictions of 
$Q$ and $f$ on $W$.
...
\begin{proof}
    ...
    spanned by $\{w_1, \ldots, w_{2m}\}$. Such an element $g$ acts on $v_2$ as
    \begin{equation*}
	\begin{array}{r@{\;}c@{\;}l}
...
\begin{proof}
    ...
    for $V$, $\mathcal{B} = \{v_1, w_1, \ldots, w_{2m-2}, v_1\}$ such that as before
    $(v_1,v_2)$ is a hyperbolic pair and $u \in \langle v_1, v_2 \rangle$, and so
    $V = \langle v_1 \rangle \oplus \langle w_1, \ldots, w_{2m-2} \rangle
	...
	\right],
    \end{equation*}
    where the matrix $A$ acts on \mbox{$W=\langle w_1, \ldots, w_{2m-2} \rangle$} as an
...
\begin{proof}
	...
	so with respect to the familiar basis $\mathcal{B} = \{v_1, w_1 ,\ldots, w_{2m}, v_2\}$,
	...
	we obtain the action on \mbox{$W = \langle w_1, \ldots, w_{2m} \rangle$} 
...
essentially similar reasoning.  % elaborate
