...
the determinant of $X \in \J$. For these purposes we define the Dickson--Freudenthal determinant
(see, for example, Section 3 in \cite{WilsonPaper})
as
\begin{equation}
...
Througout this section, let $X = (a,b,c\mid A,B,C)$ be an arbitrary element of 
...
\begin{equation}
	...
	\end{bmatrix}
\end{equation}
preserve the Dickson--Freudenthal determinant. Their actions on Albert vectors can be defined as
\begin{equation}
	\begin{array}{r@{\;}c@{\;}l}
	...
	\end{array}
\end{equation}
We call $\tau$ the \textit{triality} element. 
...
\begin{equation}
	...
	\end{bmatrix}.
\end{equation}
Note that the elements $M_x'$, $M_x''$ can be obtained from $M_x$ by conjugating by 
...
Notice that $L_x = \delta \cdot M_x \cdot \delta$, $L_x' = \delta \cdot M_x' \cdot \delta$,
and $L_x'' = \delta \cdot M_x'' \cdot \delta$, so these are also the elements of $\SE_6(F)$.
...
\end{equation}
Note that if $N(u) = 1$, then $r_u : x \mapsto -u \ovx u$ and $r_1: -u \ovx u \mapsto \ovu x \ovu$,
%It is easy to see now that the given
so the given action of $P_u$ on $\J_8^C$ is the composition $r_u \circ r_1$. 


As $u$ ranges through all 
...
Suppose $X = (a,b,c \mid A,B,C)$ and $Y = (d,e,f\mid D,E,F)$ are arbitrary Albert vectors
...
For example, for the white vector $(0,0,1\mid 0,0,0)$ the value of the quadratic form on $\J$ is 
...
\begin{lemma}
	\label{lemma:whiteness_conditions}
...
\mbox{$17$-dimensional} radical in $\J$. In the case when $Y = (0,1,1\mid 0,0,0)$ we get
...
\begin{lemma}
	...
	(0,0,0)$. In the case when $\O$ is split, $X$ can be mapped to precisely one of the
	 following:
	\begin{enumerate}[(i)]
...
\begin{proof}
	It is clear that black vectors are not in the same orbit as white and grey, 
	since they have different values of $\fdet$. We have already
	...
	\begin{equation*}
		...
		\end{array}
	\end{equation*}
	so we may choose orbit representatives with $(a,b,c) \neq (0,0,0)$. 
...
However, this is not true in all octonion algebras, which possibly leads 
to a larger number of orbits. A vector of the form $(a,b,c\mid 0,0,0)$ is 
...
surjective. This is true for any octonion algebra.
...
\begin{proof}
	...
	of $\Omega_8^+(F)$ (Lemma \ref{lemma:1_spin8plus}) 
	which in case when $\O$ is split is transitive on isotropic vectors,
	 ...
	 we require $(a,b,C) \neq (0,0,0)$ as otherwise $(a,b,c\mid A,B,C) \in \J^{17}_{cAB}$. 
	 ...
	 \begin{equation*}
	 	...
	 	\end{array}
	 \end{equation*}
	 The whiteness conditions (Lemma \ref{lemma:whiteness_conditions}) 
	 imply $C\ovC = ab$ and $BC = a\ovA$, and so additionally we have 
...
This lemma tells us precisely that the action of the stabiliser of a white point
has rank $3$ when $\O$ is split and rank $2$ when it is non-split. 
...
and some of it is contained in a series of technical lemmata. 
...
\mbox{$(a,b,0\mid 0,0,C_{-1}e_{-1} + C_1 e_1)$} with the quadratic form $\QQ_4$ obtained
as the restriction of $\QQ_{10}$ on $V$. 
...
\begin{proof}
	...
	\begin{equation*}
		\begin{array}{r@{\;}c@{\;}l}
			v_1 & = & (1,0,0 \mid 0,0,0), \\
			v_2 & = & (0,0,0 \mid 0,0,e_1), \\
			v_3 & = & (0,0,0 \mid 0,0,e_{-1}), \\
			v_4 & = & (0,1,0 \mid 0,0,0).
		\end{array}
	\end{equation*}
	It is clear that these span $V_4$, 
	so let $\mathcal{B} = (v_1,v_2,v_3,v_4)$ be the basis of $V_4$.	
	With respect to $\mathcal{B}$ the actions of $M_{\lambda e_{\pm 1}}$ can be written
	...
	\begin{equation*}
		...
		\end{bmatrix}.
	\end{equation*}
	For convenience, we do the same calculations for 
	$L_{ -\lambda e_{\pm 1}}$: its actions on the elements
	of $\mathcal{B}$ can be written as matrices 
	\begin{equation*}
		\begin{bmatrix}
			1 & 0 & 0 & 0 \\
			\lambda & 1 & 0 & 0 \\
		...
		\end{bmatrix}.
	\end{equation*}	
	We invoke Lemma \ref{lemma:B_omega4plus} to finish the proof. 
\end{proof}
...
\begin{theorem}
	...
	in case $F$ has characteristic $2$. 
\end{theorem}
...
certain groups is the following lemma, originally proved in \cite{Iwasawa}.
...
Finally, by taking $A = F^{16}$ in the Iwasawa's Lemma (Lemma \ref{lemma:iwasawa}) we obtain the following theorem.
